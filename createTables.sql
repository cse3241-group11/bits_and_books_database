...
(
    ...
    FOREIGN KEY(Warehouse_number) REFERENCES WAREHOUSE(Warehouse_number),
    FOREIGN KEY(ISBN) REFERENCES BOOK(ISBN), CHECK(length(ISBN) IN (10, 13)),
    CHECK(Book_quantity >= 0) -- CREATED THIS CONSTRAINT TO MAKE SURE NO WAREHOUSE HAS NEGATIVE BOOKS
);
...
(
    ...
    FOREIGN KEY(ISBN) REFERENCES BOOK(ISBN),
    FOREIGN KEY(ISBN, Publisher_ID) REFERENCES PUBLISHED_BY(ISBN, Publisher_ID),
    CHECK(length(ISBN) IN (10, 13))
);
